...
// Ensure down_payment column exists (run once at startup, not per-request)
let downPaymentEnsured = false;
async function ensureDownPaymentColumn() {
  if (downPaymentEnsured) return;
  try {
    await db.query(`
      ALTER TABLE products
      ADD COLUMN IF NOT EXISTS down_payment NUMERIC DEFAULT 0
    `);
    downPaymentEnsured = true;
  } catch {
