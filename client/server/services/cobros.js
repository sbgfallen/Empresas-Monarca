...
async function getAllCobros({ type, status } = {}) {
  ...
  if (cobros.length === 0) return cobros;

  // Fetch ALL payments for these cobros in a SINGLE query (fixes N+1)
  const cobroIds = cobros.map((c) => c.id);
  const paymentsResult = await db.query(
    `SELECT * FROM cobro_payments WHERE cobro_id = ANY($1) ORDER BY payment_date ASC`,
    [cobroIds]
  );

  // Group payments by cobro_id using a Map
  const paymentsByCobro = new Map();
  for (const payment of paymentsResult.rows) {
    const id = payment.cobro_id;
    if (!paymentsByCobro.has(id)) paymentsByCobro.set(id, []);
    paymentsByCobro.get(id).push(payment);
  }

  // Attach payments to each cobro
  for (const cobro of cobros) {
    cobro.payments = paymentsByCobro.get(cobro.id) || [];
  }
...
async function addPayment({ cobroId, amount, paymentDate, notes }) {
  ...
  // Auto-update cobro status to 'paid' if fully paid — single query instead of 2
  const totalCheck = await db.query(
    `UPDATE cobros SET status = 'paid', updated_at = NOW()
     WHERE id = $1 AND (
       SELECT COALESCE(SUM(amount), 0) FROM cobro_payments WHERE cobro_id = $1
     ) >= (SELECT amount FROM cobros WHERE id = $1)
     AND status = 'active'
     RETURNING id`,
    [cobroId]
  );
